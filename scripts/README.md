...
在每個交易日 **14:37 與 16:13（台北）** 觸發（兩次，因 GitHub 排程常被延遲甚至丟棄），流程：
...
GitHub 的排程不保證準時：實測 06:30 UTC 那次完全未觸發，改點後兩次分別延遲
67 與 151 分鐘。因此排兩個時段互為備援（重複執行只會以較新報價覆蓋同一份文件）。

更重要的是**期貨固定採用日盤收盤價**：夜盤 15:00 就開始，若取「最新時段」，
